...
function setupEventHandlers() {
  ...
  document.getElementById('stepModeBtn').addEventListener('click', () => setAnimationSpeed(1500));
  document.getElementById('fullSpeedBtn').addEventListener('click', () => setAnimationSpeed(500));
  document.getElementById('slowSpeedBtn').addEventListener('click', () => setAnimationSpeed(2500));
...
function resetVisualization() {
  ...
  if (window.CURRENT_DFA) {
    visualizeDFA(window.CURRENT_DFA);
...
function setAnimationSpeed(speed) {
  ...
  const speedName = speed <= 500 ? 'Fast' : speed >= 1500 ? 'Step Mode' : 'Normal';
...
function setupZoomControls() {
    ...
    const setZoom = (newZoom, centerX, centerY) => {
        ...
        const newScrollLeft = Math.round((container.scrollLeft + centerX) * scaleChange - centerX);
        const newScrollTop = Math.round((container.scrollTop + centerY) * scaleChange - centerY);
    ...
    document.getElementById("zoomInBtn").addEventListener("click", () => setZoom(zoomLevel * 1.2, container.clientWidth / 2, container.clientHeight / 2));
    document.getElementById("zoomOutBtn").addEventListener("click", () => setZoom(zoomLevel / 1.2, container.clientWidth / 2, container.clientHeight / 2));
    ...
    slider.addEventListener("input", () => setZoom(slider.value / 100, container.clientWidth / 2, container.clientHeight / 2));
}
